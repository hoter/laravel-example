 ...
 *
 *     On Success:
 *     eg. $domArr['top']['element2']['attribute']['var2'] => val2
 *
 *     On Error:
...
class XmlToArrayParser {
    ...
    public function __construct($xml) {
        ...
        xml_set_character_data_handler($this->parser, "value");
    ...
    private function tag_open($parser, $tag, $attributes) {
        $this->convert_to_array($tag, 'attribute');
        $idx=$this->convert_to_array($tag, 'value');
        if(isset($idx)) {
        ...
        if (!empty($attributes)) { $this->pointer['attribute'] = $attributes; }
    }

    /** Adds the current elements content to the current pointer[value] array. */
    private function value($parser, $cdata) { $this->pointer['value'] = trim($cdata); }
    ...
    private function tag_close($parser, $tag) {
        ...
        if(isset($current['value']) && count($current) == 1) { $current = $current['value'];}
        else if(empty($current['value'])) {unset($current['value']);}
    }
